...
export function applyCommand(current: StoredState, command: Command, now: number, ticket: number) {
  ...
  if (command.kind === "roll") {
    ...
    if (s.game.winner !== null || s.game.active === player) throw new GameError(409, "You can nudge only while waiting for the other player.");
    if (s.lastNudge && s.lastNudge.to === PLAYER_IDS[s.game.active] && now - s.lastNudge.at < 60_000) {
      throw new GameError(429, "A nudge was sent recently. Try again in a minute.");
    }
  ...
  return { state: s, event, applied: true };
}
